...
def main():
    parser = argparse.ArgumentParser(description="Benchmark Ollama models on specific AutoAgent phases.")
    parser.add_argument(
        "--models",
        nargs="*",
    ...
    if args.models:
        ...
        models_to_run = [
            m for m in all_models if any(f in m.lower() for f in families) and not any(e in m.lower() for e in exclude)
        ]
