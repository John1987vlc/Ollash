...
def app():
    ...
    mock_publisher = pytest.importorskip("backend.utils.core.event_publisher").EventPublisher()
    mock_bridge = pytest.importorskip("frontend.services.chat_event_bridge").ChatEventBridge(mock_publisher)
    mock_alerts = pytest.importorskip("backend.utils.core.alert_manager").AlertManager(mock_publisher, mock_root)
...
def test_blueprint_registration(app):
    ...
    expected_bps = [
        "prompt_studio", "audit", "knowledge", "decisions", "tuning", 
        "hil", "translator", "policies", "checkpoints", "fragments", "router"
    ]
