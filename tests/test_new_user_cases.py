...
def ollash_new_cases_agent(temp_project_root_with_config):
    agent = DefaultAgent(project_root=str(temp_project_root_with_config))
    agent.logger.logger.handlers[0].baseFilename = str(temp_project_root_with_config / "ollash_new_cases.log")
    return agent

# --- Test Cases (20 in total) ---

@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_1_ambiguous_request_clarification(mock_ollama_chat, ollash_new_cases_agent):
    ...
    mock_ollama_chat.side_effect = [
        # 1. Orchestrator tries to detect intent, gets low confidence 'exploration'
        ...
        # 2. Orchestrator provides clarification question based on its prompt
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_2_system_info_request(mock_ollama_chat, ollash_new_cases_agent):
    ...
    mock_ollama_chat.side_effect = [
        # 1. Orchestrator detects system intent, suggests switch
        ({"message": {"tool_calls": [{"function": {"name": "detect_user_intent", "arguments": {"user_request": user_request}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        ({"message": {"tool_calls": [{"function": {"name": "select_agent_type", "arguments": {"agent_type": "system"}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # 2. System agent calls get_system_info
        ({"message": {"tool_calls": [{"function": {"name": "get_system_info", "arguments": {}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # 3. System agent provides summarized output
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_3_code_refactoring_request(mock_ollama_chat, ollash_new_cases_agent):
    ...
    mock_ollama_chat.side_effect = [
        # 1. Orchestrator detects code intent, suggests switch
        ({"message": {"tool_calls": [{"function": {"name": "detect_user_intent", "arguments": {"user_request": user_request}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        ({"message": {"tool_calls": [{"function": {"name": "select_agent_type", "arguments": {"agent_type": "code"}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # 2. Code agent calls suggest_refactor
        ({"message": {"tool_calls": [{"function": {"name": "suggest_refactor", "arguments": {"file_path": "my_module.py"}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # 3. Code agent provides suggestions
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_4_network_connectivity_check(mock_ollama_chat, ollash_new_cases_agent):
    # Test: User asks to ping a host, agent switches to network and calls ping_host
    user_request = "Ping google.com for me."
    mock_ollama_chat.side_effect = [
        # 1. Orchestrator detects network intent, suggests switch
        ({"message": {"tool_calls": [{"function": {"name": "detect_user_intent", "arguments": {"user_request": user_request}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        ({"message": {"tool_calls": [{"function": {"name": "select_agent_type", "arguments": {"agent_type": "network"}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # 2. Network agent calls ping_host
        ({"message": {"tool_calls": [{"function": {"name": "ping_host", "arguments": {"host": "google.com"}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # 3. Network agent provides ping results
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_5_cybersecurity_scan_ports(mock_ollama_chat, ollash_new_cases_agent):
    ...
    mock_ollama_chat.side_effect = [
        # 1. Orchestrator detects cybersecurity intent, suggests switch
        ({"message": {"tool_calls": [{"function": {"name": "detect_user_intent", "arguments": {"user_request": user_request}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        ({"message": {"tool_calls": [{"function": {"name": "select_agent_type", "arguments": {"agent_type": "cybersecurity"}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # 2. Cybersecurity agent calls scan_ports
        ({"message": {"tool_calls": [{"function": {"name": "scan_ports", "arguments": {"host": "my_server"}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # 3. Cybersecurity agent provides scan results
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_6_mixed_domain_request(mock_ollama_chat, ollash_new_cases_agent):
    ...
    mock_ollama_chat.side_effect = [
        # 1. Orchestrator detects ambiguous intent (or low confidence exploration)
        ({"message": {"tool_calls": [{"function": {"name": "detect_user_intent", "arguments": {"user_request": user_request}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_7_loop_detection_clarification(mock_ollama_chat, ollash_new_cases_agent):
    ...
    mock_ollama_chat.side_effect = [
        # LLM will call detect_user_intent, which will return ok=True, intent='exploration'
        ({"message": {"tool_calls": [{"function": {"name": "detect_user_intent", "arguments": {"user_request": user_request}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # LLM will call detect_user_intent again
        ({"message": {"tool_calls": [{"function": {"name": "detect_user_intent", "arguments": {"user_request": user_request}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # LLM will call detect_user_intent a third time, triggering loop detection
        ({"message": {"tool_calls": [{"function": {"name": "detect_user_intent", "arguments": {"user_request": user_request}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
    ]
    ...
    # We assert that ollama.chat was called 3 times (for detect_user_intent)
    assert mock_ollama_chat.call_count == 3


@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_8_invalid_agent_type_selection(mock_ollama_chat, ollash_new_cases_agent):
    # Test: Orchestrator attempts to select an invalid agent type
    user_request = "Switch to the 'pizza' agent."
    mock_ollama_chat.side_effect = [
        # 1. Orchestrator detects intent, tries to select invalid agent
        ({"message": {"tool_calls": [{"function": {"name": "detect_user_intent", "arguments": {"user_request": user_request}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        ({"message": {"tool_calls": [{"function": {"name": "select_agent_type", "arguments": {"agent_type": "pizza"}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # 2. Orchestrator should report the error or try to re-evaluate
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_9_file_read_non_existent(mock_ollama_chat, ollash_new_cases_agent):
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_10_git_status_check(mock_ollama_chat, ollash_new_cases_agent):
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_11_install_package(mock_ollama_chat, ollash_new_cases_agent):
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_12_analyze_network_latency(mock_ollama_chat, ollash_new_cases_agent):
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_13_check_file_hash(mock_ollama_chat, ollash_new_cases_agent):
    ...
    mock_ollama_chat.side_effect = [
        ({"message": {"tool_calls": [{"function": {"name": "detect_user_intent", "arguments": {"user_request": user_request}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        ({"message": {"tool_calls": [{"function": {"name": "select_agent_type", "arguments": {"agent_type": "cybersecurity"}}}]}}, {"prompt_tokens": 10, "completion_tokens": 5}),
        # Mock check_file_hash to return a dummy value
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_14_propose_governance_policy(mock_ollama_chat, ollash_new_cases_agent):
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_15_detect_code_smells(mock_ollama_chat, ollash_new_cases_agent):
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_16_monitor_resource_spikes(mock_ollama_chat, ollash_new_cases_agent):
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_17_analyze_network_latency_multi_step(mock_ollama_chat, ollash_new_cases_agent):
    ...
    mock_ollama_chat.side_effect = [
        ...
        # Agent should remain in network context and call traceroute
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_18_analyze_security_log(mock_ollama_chat, ollash_new_cases_agent):
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_19_generate_runbook(mock_ollama_chat, ollash_new_cases_agent):
...
@patch('src.agents.default_agent.OllamaClient.chat')
def test_case_20_estimate_change_blast_radius(mock_ollama_chat, ollash_new_cases_agent):
