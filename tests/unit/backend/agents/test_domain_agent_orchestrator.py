...
def mock_architect():
    arch = MagicMock()
    arch.run = MagicMock(return_value=make_simple_dag())
    arch.plan_dag = MagicMock(return_value=make_simple_dag())
    return arch
...
def mock_developer():
    ...
    dev.run = MagicMock(return_value={"src/main.py": "def main(): pass\n"})
    return dev
...
def mock_devops():
    d = MagicMock()
    d.run = MagicMock(return_value={"Dockerfile": "FROM python\n"})
    return d
...
def mock_auditor():
    a = MagicMock()
    a.run = MagicMock(return_value={"total_files": 1, "newly_scanned": 1, "critical_files": []})
...
class TestDomainAgentOrchestrator:
    def test_run_returns_path(self, orchestrator, tmp_path):
        result = orchestrator.run("build api", "myapi")
        assert isinstance(result, Path)

    def test_architect_plan_dag_called(self, orchestrator, mock_architect):
        orchestrator.run("build api", "myapi")
        mock_architect.plan_dag.assert_called_once()

    def test_auditor_set_blackboard_called(self, orchestrator, mock_auditor):
        orchestrator.run("build api", "myapi")
        mock_auditor.set_blackboard.assert_called_once()

    def test_developer_run_called(self, orchestrator, mock_developer):
        orchestrator.run("build api", "myapi")
        assert mock_developer.run.called

    def test_checkout_developer_returns_from_pool(self, orchestrator, mock_developer):
        """Queue-based pool: checkout returns the developer and puts it back."""
        orchestrator._dev_queue.put_nowait(mock_developer)
        dev = orchestrator._checkout_developer()
        assert dev is mock_developer

    def test_checkout_and_return_developer(self, orchestrator, mock_developer):
