...
class TestProjectPlannerExtended:
    ...
    @pytest.mark.asyncio
    async def test_generate_changelog_entry_passes_project_name_in_prompt(self):
        planner, mock_llm = _make_planner()

        async def fake_prompt(project_name, changes, version=""):
            return ("sys", f"Project: {project_name}")

        with patch(f"{_PLANNER_MODULE}.AutoGenPrompts.changelog_entry_prompt", new=fake_prompt):
            await planner.generate_changelog_entry(project_name="SpecialProject", changes=["x"])
    ...
    @pytest.mark.asyncio
    async def test_generate_changelog_entry_includes_changes_in_prompt(self):
        planner, mock_llm = _make_planner()
        ...
        with patch(f"{_PLANNER_MODULE}.AutoGenPrompts.changelog_entry_prompt", new=fake_prompt):
            await planner.generate_changelog_entry(project_name="P", changes=["feature A", "fix B"])
        call_args = mock_llm.chat.call_args
        messages = call_args[1]["messages"] if call_args[1] else call_args[0][0]
        user_msg = next(m["content"] for m in messages if m["role"] == "user")
    ...
    @pytest.mark.asyncio
    async def test_generate_changelog_entry_uses_provided_version(self):
        planner, mock_llm = _make_planner()
        ...
        with patch(f"{_PLANNER_MODULE}.AutoGenPrompts.changelog_entry_prompt", new=fake_prompt):
            await planner.generate_changelog_entry(project_name="P", changes=["x"], version="2.3.0")
        call_args = mock_llm.chat.call_args
        messages = call_args[1]["messages"] if call_args[1] else call_args[0][0]
        user_msg = next(m["content"] for m in messages if m["role"] == "user")
    ...
    @pytest.mark.asyncio
    async def test_generate_roadmap_includes_gap_categories_in_prompt(self):
        planner, mock_llm = _make_planner()

        async def fake_prompt(project_name, improvement_gaps, tech_hints=None):
            return ("sys", f"Gaps: {list(improvement_gaps.keys())}")

        with patch(f"{_PLANNER_MODULE}.AutoGenPrompts.roadmap_prompt", new=fake_prompt):
            await planner.generate_roadmap(
                project_name="P",
    ...
    @pytest.mark.asyncio
    async def test_generate_roadmap_includes_tech_hints_when_provided(self):
        planner, mock_llm = _make_planner()
        ...
        with patch(f"{_PLANNER_MODULE}.AutoGenPrompts.roadmap_prompt", new=fake_prompt):
            await planner.generate_roadmap(project_name="P", improvement_gaps={}, tech_stack_info=mock_stack)
        call_args = mock_llm.chat.call_args
        messages = call_args[1]["messages"] if call_args[1] else call_args[0][0]
        user_msg = next(m["content"] for m in messages if m["role"] == "user")
    ...
    @pytest.mark.asyncio
    async def test_update_readme_summary_passes_existing_readme_in_prompt(self):
        planner, mock_llm = _make_planner()

        async def fake_prompt(existing_readme, cycle_summary):
            return ("sys", f"Readme: {existing_readme}")

        with patch(f"{_PLANNER_MODULE}.AutoGenPrompts.readme_summary_update_prompt", new=fake_prompt):
            await planner.update_readme_summary(
    ...
    @pytest.mark.asyncio
    async def test_update_readme_summary_passes_cycle_summary_in_prompt(self):
        planner, mock_llm = _make_planner()
