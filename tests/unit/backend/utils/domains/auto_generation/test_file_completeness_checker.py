...
def test_verify_and_fix_all_valid(checker, mock_validator):
    files = {"main.py": "print('hello')"}
    mock_validator.validate_batch.return_value = [ValidationResult("main.py", ValidationStatus.VALID, "OK", 1, 14)]

    result = checker.verify_and_fix(files)
    assert result == files
    assert checker.llm_client.chat.call_count == 0


def test_verify_and_fix_empty_file_triggers_generation(checker, mock_llm_client, mock_validator, mock_parser):
    files = {"empty.py": ""}
    ...
    mock_llm_client.chat.return_value = ({"message": {"content": "print('generated')"}}, {})
    mock_parser.extract_code.return_value = "print('generated')"

    result = checker.verify_and_fix(files, "readme context")

    assert result["empty.py"] == "print('generated')"
    assert mock_llm_client.chat.call_count == 1


def test_verify_and_fix_failed_file_triggers_fix(checker, mock_llm_client, mock_validator, mock_parser):
    ...
    mock_llm_client.chat.return_value = ({"message": {"content": "def f():\n    pass"}}, {})
...
def test_verify_and_fix_max_retries_reached(checker, mock_llm_client, mock_validator, mock_parser):
    ...
    mock_llm_client.chat.return_value = ({"message": {"content": "attempt"}}, {})
    ...
    assert mock_llm_client.chat.call_count == 2  # max_retries_per_file is 2 in fixture
