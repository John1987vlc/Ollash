...
class TestEventPublisher:
    ...
    async def test_subscribe_and_publish(self, publisher):
        callback = MagicMock()
    ...
    async def test_multiple_subscribers(self, publisher):
        ...
        await publisher.publish("event", {"data": 1})
    ...
    async def test_publish_kwargs(self, publisher):
        callback = MagicMock()
        publisher.subscribe("event", callback)

        await publisher.publish("event", direct_data={"a": 1}, extra="info")
    ...
    async def test_callback_exception_isolation(self, publisher):
