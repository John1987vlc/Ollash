...
@pytest.fixture
def mock_phase_context(mocker, tmp_path, mock_logger, mock_event_publisher,
                       mock_llm_manager, mock_file_manager):
    ...
    ctx.parallel_generator.get_statistics.return_value = {
        "success": 0, "total": 0, "avg_time_per_file": 0.0
    }
...
@pytest.fixture
def auto_agent_instance(mocker, test_kernel, mock_phase_context, mock_phases,
                        mock_llm_manager, mock_llm_recorder,
                        mock_event_publisher, mock_execution_plan):
    ...
    mock_loop.run_until_complete.side_effect = lambda coro: real_loop.run_until_complete(coro) if asyncio.iscoroutine(coro) else coro
...
class TestAutoAgentRun:
    ...
    def test_run_existing_project_ingests(self, auto_agent_instance, mock_phase_context, mocker):
        ...
        mock_phase_context.ingest_existing_project.return_value = (
            {"main.py": "print('hello')"}, {"main.py": {}}, ["main.py"]
        )

        agent.run("Improve the calculator", project_name)
        mock_phase_context.ingest_existing_project.assert_called_once()

    def test_run_writes_execution_plan_json(self, auto_agent_instance, mock_phase_context,
                                            mock_execution_plan):
...
class TestPhaseContextInferLanguage:
    ...
    @pytest.mark.parametrize("source,lang,expected_pattern", [
    ...
    def test_implement_plan_create_file(self, real_phase_context, tmp_path):
        plan = {
            "actions": [
                {"type": "create_file", "path": "new_file.py", "content": "print('new')"},
            ]
        }
    ...
    def test_implement_plan_modify_file(self, real_phase_context, tmp_path):
        plan = {
            "actions": [
                {"type": "modify_file", "path": "app.py", "changes": {"old_func": "new_func"}},
            ]
        }
...
class TestSeniorReviewPhase:
    ...
    async def test_review_retries_on_failure(self, mock_phase_context, tmp_path):
        ...
        mock_phase_context.senior_reviewer.perform_review.side_effect = [
            {"status": "failed", "issues": [{"file": "main.py", "description": "Bug", "severity": "high", "recommendation": "Fix it"}], "summary": "Needs work"},
    ...
    async def test_review_fails_all_attempts(self, mock_phase_context, tmp_path):
        ...
        mock_phase_context.senior_reviewer.perform_review.return_value = {
            "status": "failed", "issues": [], "summary": "Cannot fix"
        }
...
class TestTestGenerationExecutionPhase:
    ...
    async def test_generates_and_executes_tests(self, mock_phase_context, tmp_path):
        ...
        mock_phase_context.test_generator.generate_integration_tests.return_value = (None, None)
    ...
    async def test_retries_on_test_failure(self, mock_phase_context, tmp_path):
        ...
        mock_phase_context.test_generator.execute_tests.side_effect = [
            {"success": False, "failures": [{"path": "app.py", "message": "Exception raised"}]},
            {"success": False, "failures": [{"path": "app.py", "message": "Still broken"}]},
            {"success": True},
        ]
    ...
    async def test_skips_test_files_from_generation(self, mock_phase_context, tmp_path):
        ...
        await phase.execute(
            ...
            readme_content="",
            initial_structure={},
            generated_files={"app.py": "def add(): pass", "test_app.py": "def test_add(): pass"},
...
class TestExhaustiveReviewRepairPhase:
    ...
    def test_generate_repair_report(self, mock_phase_context):
        ...
        report = phase._generate_repair_report(
            diagnostics={"coherence_score": 0.8, "critical_issues": [
                {"severity": "critical", "file": "main.py", "description": "Missing import"}
            ]},
            predicted_errors=[],
...
class TestIterativeImprovementPhase:
    ...
    async def test_runs_improvement_loops(self, mock_phase_context, tmp_path):
        ...
        mock_phase_context.implement_plan.return_value = ({"app.py": "improved"}, {}, ["app.py"])
...
requires_ollama = pytest.mark.skipif(
    ...
    reason="Ollama server not available at " + TEST_OLLAMA_URL,
)

requires_ministral = pytest.mark.skipif(
    not _model_available("ministral"),
    reason="ministral-3:8b model not available",
)

requires_qwen_coder = pytest.mark.skipif(
    not _model_available("qwen2.5-coder"),
    reason="qwen2.5-coder:7b model not available",
)


@requires_ollama
class TestIntegrationAutoAgentWithOllama:
    """Integration tests that use real local LLMs via Ollama."""

    @pytest.fixture
    def integration_kernel(self, tmp_path, monkeypatch):
        """Kernel configured for real Ollama interaction."""
        models_config = {
            "ollama_url": TEST_OLLAMA_URL,
            "default_model": INTEGRATION_MODELS["primary"],
            "default_timeout": 300,
            "agent_roles": {
                "prototyper": INTEGRATION_MODELS["primary"],
                "coder": INTEGRATION_MODELS.get("coder", INTEGRATION_MODELS["primary"]),
                "planner": INTEGRATION_MODELS["primary"],
                "generalist": INTEGRATION_MODELS["primary"],
                "suggester": INTEGRATION_MODELS["primary"],
                "improvement_planner": INTEGRATION_MODELS["primary"],
                "senior_reviewer": INTEGRATION_MODELS["primary"],
                "test_generator": INTEGRATION_MODELS.get("coder", INTEGRATION_MODELS["primary"]),
                "default": INTEGRATION_MODELS["primary"],
            },
        }
        ...
        monkeypatch.setenv("TOOL_SETTINGS_JSON", json.dumps({
            "log_file": "test.log",
            "completeness_checker_max_retries": 1,
            "senior_review_max_attempts": 1,
        }))
    ...
    def test_real_readme_generation(self, integration_kernel, tmp_path):
        """Use real LLM to generate a README via the container."""
        main_container.wire(modules=[__name__, "backend.agents.auto_agent"])

        with main_container.core.agent_kernel.override(integration_kernel):
            agent: AutoAgent = main_container.auto_agent_module.auto_agent()

        main_container.unwire()

        ctx = agent.phase_context
        readme = ctx.project_planner.generate_readme(
            "A Python CLI tool that converts CSV files to JSON format",
            "default", "3.12", "MIT", False,
        )

        assert readme is not None
        assert len(readme) > 50
        assert any(kw in readme.lower() for kw in ["csv", "json", "convert", "python", "cli"])

    @requires_ministral
    @pytest.mark.timeout(120)
    def test_real_logic_planning(self, integration_kernel, tmp_path):
        """Use real LLM to create logic plans."""
        main_container.wire(modules=[__name__, "backend.agents.auto_agent"])
        ...
        ctx = agent.phase_context
        phase = LogicPlanningPhase(ctx)

        loop = asyncio.new_event_loop()
        try:
            files, structure, paths = loop.run_until_complete(
                phase.execute(
                    project_description="A REST API for managing a todo list",
                    project_name="todo_api",
                    project_root=tmp_path,
                    readme_content="# Todo API\nA REST API for todo management.",
                    initial_structure={},
                    generated_files={},
                    file_paths=["app.py", "models.py", "routes.py"],
                )
            )
        finally:
            loop.close()
        ...
        assert "IMPLEMENTATION_PLAN.json" in files
        plan_data = json.loads(files["IMPLEMENTATION_PLAN.json"])
        assert isinstance(plan_data, dict)

    @requires_qwen_coder
    @pytest.mark.timeout(180)
    def test_real_structure_generation(self, integration_kernel, tmp_path):
        """Test generate_structure_only with real LLM."""
        main_container.wire(modules=[__name__, "backend.agents.auto_agent"])
        ...
        readme, structure = agent.generate_structure_only(
            "A simple Python calculator with add, subtract, multiply, divide operations",
            "simple_calc",
        )
    ...
    @requires_ministral
    @pytest.mark.timeout(120)
    def test_real_senior_review(self, integration_kernel, tmp_path):
        """Use real LLM for senior review of a simple project."""
        main_container.wire(modules=[__name__, "backend.agents.auto_agent"])
        ...
        ctx = agent.phase_context
        review_result = ctx.senior_reviewer.perform_review(
            project_description="A calculator",
            project_name="calc",
            readme_content="# Calculator\nSimple arithmetic operations.",
            json_structure={"main.py": {}},
            current_files={
                "main.py": """
def add(a, b):
    return a + b
...
"""
        ...
        assert review_result["status"] in ("passed", "failed")
