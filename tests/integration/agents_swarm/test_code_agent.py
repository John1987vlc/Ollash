...
def test_code_agent_initialization(default_agent):
...
def test_list_directory_tool(default_agent, tmp_path):
    """Test the list_directory tool."""
    import asyncio

    (tmp_path / "ollash_test_project" / "test_file.txt").touch()
    result = asyncio.run(default_agent.tool_executor.execute_tool("list_directory", path="."))
    assert "test_file.txt" in result["items"]


def test_read_file_tool(default_agent, tmp_path):
    """Test the read_file tool."""
    import asyncio

    test_file = tmp_path / "ollash_test_project" / "test_file.txt"
    test_file.write_text("hello world")
    result = asyncio.run(default_agent.tool_executor.execute_tool("read_file", path="test_file.txt"))
    assert result["content"] == "hello world"


def test_write_file_tool_confirmation_yes(default_agent, tmp_path, monkeypatch):
    ...
    content = "new content"
    result = asyncio.run(
        default_agent.tool_executor.execute_tool("write_file", path=file_path, content=content, reason="test")
    )
    assert result["ok"]
    assert (tmp_path / "ollash_test_project" / file_path).read_text() == content


def test_write_file_tool_confirmation_no(default_agent, tmp_path, monkeypatch):
