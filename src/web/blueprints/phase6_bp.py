...
def get_decision_suggestions():
    ...
    return jsonify({
        ...
        "suggestions": [
            {
                "decision": str(s),
                "similarity": getattr(s, 'similarity_score', 0.5)
            }
...
def register_trigger():
    ...
    from src.utils.core.advanced_trigger_manager import CompositeTriggerCondition
...
def batch_operation():
    ...
    for op in operations:
        ...
        if op_type == 'send_notification':
            webhook_mgr = get_webhook_manager()
            success = webhook_mgr.send_to_webhook_sync(
                webhook_name=op_data.get('webhook'),
