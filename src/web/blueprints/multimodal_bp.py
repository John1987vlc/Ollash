...
from src.utils.core.ocr_processor import OCRProcessor, PDFOCRProcessor, OCRConfig
from src.utils.core.multimedia_ingester import MultimediaIngester, DocumentType
...
ocr_processor = None
multimedia_ingester = None
speech_transcriber = None
logger = None


def init_app(app, workspace_path: str = "knowledge_workspace"):
    """Initialize multimodal managers"""
    global ocr_processor, multimedia_ingester, speech_transcriber, logger
    
    logger = StructuredLogger("phase5_multimodal")
    
    # Initialize OCR processor
    ocr_processor = OCRProcessor(workspace_path=workspace_path)
    logger.info("OCRProcessor initialized")
    
    # Initialize multimedia ingester
    multimedia_ingester = MultimediaIngester(workspace_path=workspace_path)
    multimedia_ingester.set_ocr_processor(ocr_processor)
    logger.info("MultimediaIngester initialized")
    
    # Initialize speech transcriber
    speech_transcriber = SpeechTranscriber(workspace_path=workspace_path)
...
def ingest_normalize():
    ...
    try:
        ...
        from src.utils.core.multimedia_ingester import ParsedDocument, ContentBlock
