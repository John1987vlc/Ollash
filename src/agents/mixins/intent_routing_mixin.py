...
from src.interfaces.imodel_provider import IModelProvider # Assuming LLMClientManager implements IModelProvider
    ...
    async def _classify_intent(self, prompt: str) -> str:
        ...
        if orchestration_client:
            try:
                ...
                messages = [
                    {"role": "system", "content": "You are an intent classifier. Given a user prompt, classify its primary intent as one of: 'code', 'network', 'system', 'cybersecurity', 'planning', 'general', 'prototyping', 'testing', 'reviewing', 'analysing', 'writing'."},
...
Classify the intent:"""}
            ...
            self.logger.warning("Orchestration LLM client not available for intent classification. Falling back to default.")
            return "default"
    ...
    def _select_model_for_intent(self, intent: str) -> Optional[Any]:
        ...
        model_role_map = self.config.get("intent_to_model_role_map", {
            "code": "coder",
