...
class ModelRouter:
    ...
    async def aroute_and_aggregate(
        ...
        for i, role in enumerate(roles_in_task_order):
            ...
            if isinstance(result, Exception):
                ...
                self.event_publisher.publish("tool_output", tool_name="model_router", model=client.model, role=role, status="error", message=str(result))
            else:
                response_data, usage = result
                candidate_responses.append((role, response_data))
                self.event_publisher.publish("tool_output", tool_name="model_router", model=client.model, role=role, status="success", content=response_data["message"].get("content", "")[:200])
        ...
        try:
            ...
            self.event_publisher.publish("tool_output", tool_name="senior_reviewer_selection", status="success", choice=choice_content[:200])
        except Exception as e:
            self.logger.error(f"  Error getting response from Senior Reviewer: {e}")
            self.event_publisher.publish("tool_output", tool_name="senior_reviewer_selection", status="error", message=str(e))
    ...
    def route_and_aggregate(
        ...
        for role in candidate_model_roles:
            ...
            try:
                ...
                self.event_publisher.publish("tool_output", tool_name="model_router", model=client.model, role=role, status="error", message=str(e))
            self.event_publisher.publish("tool_end", tool_name="model_router", model=client.model, role=role)
    ...
    def _parse_reviewer_choice(self, choice_content: str, candidate_responses: List[Tuple[str, Dict]]) -> Dict:
        ...
        for role, response_data in candidate_responses:
            ...
            if response_data["message"].get("tool_calls") and json.dumps(response_data["message"]["tool_calls"]) == choice_content:
