...
from src.utils.core.metrics_database import get_metrics_database
from src.utils.core.trigger_manager import get_trigger_manager
...
class AutomationTaskExecutor:
    ...
    async def execute_task(
        ...
        try:
            ...
            self.event_publisher.publish('task:completed', {
                'task_id': task_id,
                'task_name': task_name,
                'output': output
            })
            ...
            # Publish error event
            self.event_publisher.publish('task:error', {
                'task_id': task_id,
                'task_name': task_name,
                'error': error_msg
            })
