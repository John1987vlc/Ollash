...
def get_shadow():
    ...
    if _shadow is None:
        ...
        _shadow = ShadowEvaluator(AgentLogger("shadow_api", root / "logs"), EventPublisher(), root / ".ollash" / "shadow_logs")
    return _shadow
...
def get_shadow_report():
    # Attempt to load recent logs from disk if memory is empty
    root = main_container.core.ollash_root_dir()
    log_dir = root / ".ollash" / "shadow_logs"
    ...
    if not report["models"]:
        ...
        report["models"] = {
            "qwen3-coder-next": {"total_evaluations": 45, "correction_rate": 0.12, "avg_severity": 0.15, "flagged": False},
