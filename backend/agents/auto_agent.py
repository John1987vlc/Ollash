...
class AutoAgent(CoreAgent):
    ...
    def __init__(self,
                 ...
                 project_analysis_phase_factory: providers.Factory[ProjectAnalysisPhase] = Provide['auto_agent_module.project_analysis_phase_factory'],
    ...
    def run(self, project_description: str, project_name: str = "new_project", **kwargs) -> Path:
        ...
        execution_plan = self._setup_and_run_phases(active_phases, project_description, project_name, project_root, project_exists, readme_content, initial_structure, generated_files, file_paths)

        self._finalize_project(project_name, project_root, len(file_paths), execution_plan)
        return project_root

    def _setup_and_run_phases(self, phases: List[IAgentPhase], project_description: str, project_name: str, project_root: Path, project_exists: bool, readme_content: str, initial_structure: dict, generated_files: dict, file_paths: list) -> ExecutionPlan:
        ...
        for phase in phases:
            ...
            try:
                ...
                generated_files, initial_structure, file_paths = loop.run_until_complete(phase.execute(
                    project_description=project_description, project_name=project_name, project_root=project_root,
                    readme_content=readme_content, initial_structure=initial_structure,
                    generated_files=generated_files, file_paths=file_paths, **self.phase_context.initial_exec_params
                ))
    ...
    async def _run_structure_phases_async(self, phases: List[IAgentPhase], project_description: str, project_name: str) -> Tuple[str, Dict]:
