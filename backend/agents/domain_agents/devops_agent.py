...
class DevOpsAgent(BaseDomainAgent):
    ...
    def run(
        ...
        try:
            infra_files = self._generate_infra(
        ...
        if ci_failures:
            healed = self._heal_ci_failures(ci_failures, project_name)
            infra_files.update(healed)

        # 3 — Write to Blackboard
        for rel_path, content in infra_files.items():
            blackboard.write_sync(f"infra_files/{rel_path}", content, self.agent_id)
    ...
    def _generate_infra(
        ...
        """Delegate to InfraGenerator (sync)."""
        try:
            result = self._infra_gen.generate(
                project_name=project_name,
                project_description=project_description,
                generated_files=generated_files,
            )
    ...
    def _heal_ci_failures(
        ...
        for failure in ci_failures:
            ...
            if not workflow_log:
                continue
            try:
                analysis = self._cicd_healer.analyze_failure(
