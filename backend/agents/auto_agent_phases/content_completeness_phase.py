...
class ContentCompletenessPhase(IAgentPhase):
    ...
    async def execute(self,
        ...
        if incomplete_files:
            self.context.logger.info(f"  Found {len(incomplete_files)} incomplete files, attempting to complete them...")
            self.context.event_publisher.publish("tool_start", tool_name="complete_incomplete_files", count=len(incomplete_files))
            for rel_path in incomplete_files:
                ...
                try:
                    ...
                    if refined:
                        ...
                        self.context.event_publisher.publish("tool_output", tool_name="complete_incomplete_files", file=rel_path, status="success")
                except Exception as e:
                    self.context.logger.error(f"    Error completing {rel_path}: {e}")
                    self.context.event_publisher.publish("tool_output", tool_name="complete_incomplete_files", file=rel_path, status="error", message=str(e))
        ...
        self.context.event_publisher.publish("phase_complete", phase="7.5", message="Content completeness check complete")
