    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        code_files = {
            ...
            if content and self._is_scannable(path)
        }

        scan_report = self.context.vulnerability_scanner.scan_project(
            code_files, block_on_critical=False
        )
        ...
        self.context.file_manager.write_file(
            project_root / "SECURITY_SCAN_REPORT.md", report_content
        )
        ...
        if dependabot_config:
            ...
            if ".github/dependabot.yml" not in generated_files:
                ...
                self.context.file_manager.write_file(
                    github_dir / "dependabot.yml", dependabot_config
                )
        ...
        if block_on_critical and scan_report.critical_count > 0:
            self.context.logger.error(
                f"SECURITY SCAN: {scan_report.critical_count} CRITICAL vulnerabilities found!"
            )
            self.context.event_publisher.publish(
                "phase_complete",
                phase="security_scan",
    ...
    def _format_scan_report(self, report, project_name: str) -> str:
        ...
        for result in report.file_results:
            ...
            for vuln in result.vulnerabilities:
                lines.append(
                    f"- **[{vuln.severity.upper()}]** {vuln.description} "
                    f"(line {vuln.line_number}) — {vuln.rule_id}"
                )
