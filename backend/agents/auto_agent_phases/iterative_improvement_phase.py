...
class IterativeImprovementPhase(IAgentPhase):
    ...
    async def execute(self,
        ...
        if num_refine_loops > 0:
            ...
            self.context.event_publisher.publish("phase_start", phase="7", message=f"Starting iterative improvement loops ({num_refine_loops} loops)")
            for loop_num in range(num_refine_loops):
                ...
                self.context.event_publisher.publish("tool_start", tool_name="suggest_improvements", iteration=loop_num + 1)
                suggestions = self.context.improvement_suggester.suggest_improvements(
                    project_description, readme_content, initial_structure, generated_files, loop_num
                )
                self.context.logger.info(f"  Suggested Improvements ({len(suggestions)}): {', '.join(suggestions[:3])}...")
                self.context.event_publisher.publish("tool_output", tool_name="suggest_improvements", suggestions=suggestions)
                ...
                if not suggestions:
                    ...
                    self.context.event_publisher.publish("iteration_end", phase="7", iteration=loop_num + 1, message="No further improvements suggested")
                    break

                # 2. Plan improvements
                self.context.event_publisher.publish("tool_start", tool_name="plan_improvements", iteration=loop_num + 1)
                plan = self.context.improvement_planner.generate_plan(
                    suggestions, project_description, readme_content, initial_structure, generated_files
                )
                if not plan or not plan.get("actions"):
                    self.context.logger.warning("  Improvement plan could not be generated or was empty. Skipping this iteration.")
                    self.context.event_publisher.publish("tool_output", tool_name="plan_improvements", status="failed", message="Improvement plan empty")
                    self.context.event_publisher.publish("tool_end", tool_name="plan_improvements")
                    self.context.event_publisher.publish("iteration_end", phase="7", iteration=loop_num + 1, status="skipped")
                    continue
                self.context.logger.info(f"  Improvement Plan generated with {len(plan.get('actions', []))} actions.")
                self.context.event_publisher.publish("tool_output", tool_name="plan_improvements", status="success", plan=plan)
                self.context.event_publisher.publish("tool_end", tool_name="plan_improvements")
                ...
                generated_files, initial_structure, file_paths = self.context.implement_plan(
                    plan, project_root, readme_content, initial_structure, generated_files, file_paths
                )
                self.context.event_publisher.publish("tool_output", tool_name="implement_plan", status="success", files_updated=len(generated_files))
                ...
                for idx, (rel_path, content) in enumerate(list(generated_files.items()), 1):
                    if not content or len(content) < 10:
                        continue
                    self.context.event_publisher.publish("tool_start", tool_name="file_refinement", file=rel_path, progress=f"{idx}/{len(file_paths)}", iteration=loop_num + 1)
                ...
                self.context.event_publisher.publish("phase_complete", phase="5_rerun", iteration=loop_num + 1, message="Refinement re-run complete")
                ...
                self.context.event_publisher.publish("phase_complete", phase="5.5_rerun", iteration=loop_num + 1, message="Verification re-run complete")
                self.context.event_publisher.publish("iteration_end", phase="7", iteration=loop_num + 1, status="complete")
