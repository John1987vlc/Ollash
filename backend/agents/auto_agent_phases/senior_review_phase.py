        ...
        **kwargs: Any,
        ...
        while not review_passed and review_attempt < max_review_attempts:
            ...
            self.context.logger.info(
                f"PHASE 8: Senior Review Attempt {review_attempt}/{max_review_attempts}..."
            )
            self.context.event_publisher.publish(
            ...
            if review_results.get("status") == "passed":
                ...
                generated_files[review_summary_path] = review_results.get(
                    "summary", "Senior review passed."
                )
                ...
                self.context.logger.warning(
                    f"PHASE 8: Senior Review Failed. Issues found: {len(issues)}"
                )
                self.context.event_publisher.publish(
                    "tool_output",
                    tool_name="senior_review",
...
"""
                ...
                self.context.file_manager.write_file(
                    project_root / issue_log_path, generated_files[issue_log_path]
                )

                if issues:
                    self.context.logger.info(
                        "  Attempting targeted fixes based on senior review issues..."
                    )
                    self.context.event_publisher.publish(
                        "tool_start",
                    ...
                    contingency_plan = (
                        self.context.contingency_planner.generate_contingency_plan(
                            issues, project_description, readme_content
                        )
                    )
                    if contingency_plan and contingency_plan.get("actions"):
                        self.context.logger.info(
                            "  Contingency plan generated. Implementing..."
                        )
                        (
                            generated_files,
                            initial_structure,
                        ...
                        for rel_path in files_with_issues:
                            ...
                            try:
                                file_issues = [
                                    iss for iss in issues if iss.get("file") == rel_path
                                ]
                                ...
                                if refined:
                                    ...
                                    self.context.logger.warning(
                                        f"    Refiner failed to improve {rel_path}."
                                    )
                                    self.context.event_publisher.publish(
                                        "tool_output",
                                        tool_name="refine_from_senior_review",
                        ...
                        if general_issues:
                            ...
                            for rel_path, content in list(generated_files.items()):
                                if (
                                    not content
                                    or len(content) < 10
                                    or rel_path in files_with_issues
                                ):
                                    continue
                                self.context.event_publisher.publish(
                                    "tool_start",
                    ...
                    self.context.logger.info(
                        "  Re-running verification after senior review fixes..."
                    )
                    generated_files = (
                        self.context.file_completeness_checker.verify_and_fix(
                    ...
                    self.context.logger.warning(
                        "  No specific issues provided by senior reviewer to fix."
                    )
        ...
        if not review_passed:
            ...
            if review_attempt >= 2:
                self.context.logger.warning(
                    "  PHASE 8: Attempting aggressive simplification to resolve issues..."
                )
                self.context.event_publisher.publish(
                ...
                for rel_path, content in list(generated_files.items()):
                    if (
                        content
                        and len(content) > 100
                        and not rel_path.endswith((".md", ".json", ".yml", ".yaml"))
                    ):
                        try:
                            ...
                            if simplified and len(simplified) < len(content):
                                ...
                                self.context.file_manager.write_file(
                                    project_root / rel_path, simplified
                                )
                                simplified_count += 1
                                self.context.logger.info(f"    ✓ Simplified {rel_path}")
                        except Exception as e:
                            self.context.logger.debug(
                                f"    Simplification skipped for {rel_path}: {e}"
                            )

                self.context.logger.info(
                    f"  Simplified {simplified_count} files to improve stability"
                )
                self.context.event_publisher.publish(
                    "tool_end",
        ...
        self.context.logger.info(
            f"Project '{project_name}' completed at {project_root}"
        )
        self.context.event_publisher.publish(
            "project_complete",
