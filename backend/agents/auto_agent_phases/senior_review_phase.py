...
"""
        ...
        if senior_review_as_pr and git_token and review_passed is not None:
            self._create_review_pr(
                project_root,
                project_name,
                review_results if not review_passed else {"status": "passed", "summary": "All checks passed", "issues": []},
                generated_files,
            )
    ...
    def _create_review_pr(
        ...
        try:
            ...
            if pr_result.success and pr_result.pr_number:
                ...
                self.context.logger.warning(
                    f"  Review PR creation failed: {pr_result.error}"
                )
...
"""
        ...
        body += (
            f"---\n"
            f"*Generated by Ollash AutoAgent Senior Review*\n"
            f"Files analyzed: {len(generated_files)}"
        )
    ...
    def _post_review_comments(
        self, project_root: Path, pr_number: int, issues: List[Dict]
    ) -> None:
