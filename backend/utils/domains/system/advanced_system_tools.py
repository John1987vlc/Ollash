        ...
        try:
            if self.os_type == "Windows":
                ...
                if resource_type == "cpu":
                    ...
                    if result.success:
                        io_match = re.search(
                            r"avg-cpu:\s+%.*?%idle\s+(\d+\.?\d*)\s+Device:\s+.*?\s+(\d+\.?\d*)\s+(\d+\.?\d*)",
                            result.stdout,
                            re.DOTALL,
                        )
                        if io_match:
            ...
            return {
                "ok": False,
                "result": {"error": str(e), "resource_type": resource_type},
            }
        ...
        try:
            ...
            if not result_pre_reset.success:
                return {
                    "ok": False,
                    "result": {
                        "error": "Failed to get current git HEAD.",
                        "details": result_pre_reset.stderr,
                    },
                }
            ...
            if not result.success:
                return {
                    "ok": False,
                    "result": {
                        "error": "git reset --hard HEAD~1 failed.",
            ...
            if not result_post_reset.success:
                return {
                    "ok": True,
                    "result": {
                        "status": "rollback_likely_successful_but_unverified",
                        "details": "git reset command succeeded, but failed to verify the new HEAD.",
                    },
                }
            ...
            if head_before_reset == head_after_reset:
                return {
                    "ok": False,
                    "result": {
                        "error": "Rollback failed. HEAD is still at the same commit.",
                        "details": head_before_reset,
                    },
                }
