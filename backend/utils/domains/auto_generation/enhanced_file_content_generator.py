...
'''
    ...
    def _apply_partial_edits(self,
                            file_path: str,
                            current_content: str,
                            readme: str,
                            issues_to_fix: List[Dict]) -> str:
        """
    ...
    def _merge_original_with_improvements(self,
                                         file_path: str,
                                         current_content: str,                                          readme: str) -> str:
        """
...
Output the improved version only, no explanations."""
    ...
    def _generate_section_fix(self,
                             file_path: str,
                             problem_section: str,
                             issue_desc: str,
                             readme: str) -> str:
...
Generate ONLY the fixed code (same language), no explanations."""
        ...
        try:
            response_data, _ = self.llm_client.chat(
                messages=[
                    {"role": "system", "content": "You are a code fixer. Fix issues precisely."},
    ...
    def _is_better_line(self, orig: str, improved: str) -> bool:
        ...
        if improved.count('#') + improved.count('//') + improved.count('"""') > orig.count('#') + orig.count('//') + orig.count('"""'):
            return True
