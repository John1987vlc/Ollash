...
class TestGenerator:
    ...
    def execute_tests(self, project_root: Path, test_file_paths: List[Path]) -> Dict[str, Any]:
        ...
        pytest_command = ["pytest", "--json-report", "--json-report-file=.pytest_report.json", "-s"] + [str(p.relative_to(project_root)) for p in test_file_paths]
        ...
        try:
            ...
            if result.success:
                ...
                return {"success": True, "output": result.stdout, "failures": self._parse_json_report_failures(report_data)}
            else:
                self.logger.warning(f"  Tests failed for {len(test_file_paths)} files.")
                return {"success": False, "output": result.stderr or result.stdout, "failures": self._parse_json_report_failures(report_data)}
