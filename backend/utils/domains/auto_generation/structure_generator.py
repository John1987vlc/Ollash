...
class StructureGenerator:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not structure["folders"] and not structure["files"]:
                    raise ValueError(
                        "High-level structure is empty. Rellamando a la funcion para regenerar el JSON de la estructura del proyecto"
                    )
    ...
    def create_empty_files(project_root: Path, json_structure: dict, current_path: str = ""):
        ...
        for file_name in json_structure.get("files", []):
            ...
            if file_path.is_dir():
                print(
                    f"WARNING: Attempted to create file '{file_path}' but a directory with that name already exists. Skipping file creation."
                )
                continue
        ...
        for folder_data in json_structure.get("folders", []):
            ...
            if folder_name:
                ...
                if new_path_full.is_file():
                    print(
                        f"WARNING: Attempted to create directory '{new_path_full}' but a file with that name already exists. Skipping directory creation."
                    )
                    continue
                        ...
                        "folders": [
                            {
                                "name": "api",
                                "folders": [],
                                "files": ["__init__.py", "v1", "routers.py"],
                            },
                            {
                                "name": "core",
                                "folders": [],
                ...
                "folders": [
                    {
                        ...
                        "folders": [
                            {"name": "assets", "folders": [], "files": ["logo.svg"]},
                            {
                                "name": "components",
                                "folders": [],
                                "files": ["Button.jsx", "Header.jsx"],
                            },
                            {
                                "name": "pages",
                                "folders": [],
                                "files": ["HomePage.jsx", "AboutPage.jsx"],
                            },
                        ],
