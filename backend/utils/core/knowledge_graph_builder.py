...
class KnowledgeGraphBuilder:
    ...
    def get_concept_connections(self, term: str, max_depth: int = 2) -> Dict[str, Any]:
        """
    ...
    def _add_section_node(self, section: Dict, doc_path: Path) -> str:
        ...
        if section_id not in self.nodes:
            node = GraphNode(
                ...
                label=section["title"],
                node_type="section",
                metadata={"document": str(doc_path.name), "level": section["level"]},
                created_at=self._get_timestamp(),
            )
    ...
    def _add_term_node(self, term: str, context: str) -> str:
        ...
        if node_id not in self.nodes:
            node = GraphNode(
                ...
                label=term,
                node_type="term",
                metadata={"appearances": 1, "theme": self._infer_theme(term, context)},
                created_at=self._get_timestamp(),
            )
            self.nodes[node_id] = node
    ...
    def _traverse_graph(
        ...
        connections = {"edges": [], "depth": current_depth}
