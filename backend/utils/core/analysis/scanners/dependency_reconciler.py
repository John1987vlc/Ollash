...
class DependencyReconciler:
    ...
    def reconcile(
        ...
        try:
            ...
            self._logger.warning(
                f"DependencyScanner error, falling back to basic reconciliation: {exc}"
            )
    ...
    def _reconcile_python_requirements(
        ...
        req_key = next(
            (p for p in files if Path(p).name == "requirements.txt"), None
        )
        if not req_key:
            return files
    ...
    def _reconcile_go_mod(
        ...
        if len(require_lines) > max(len(scanned_modules) * 3, 30):
            self._logger.info(
                f"  Regenerating {mod_key} require block: "
                f"{len(require_lines)} → {len(scanned_modules)} modules"
            )
    ...
    def _reconcile_cargo_toml(
        ...
        if len(dep_lines) > max(len(scanned_crates) * 3, 20):
            self._logger.info(
                f"  Trimming {cargo_key} [dependencies]: "
                f"{len(dep_lines)} → {len(scanned_crates)} crates"
            )
