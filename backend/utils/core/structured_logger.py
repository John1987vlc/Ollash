    ...
    def format(self, record):
        ...
        for key, value in record.__dict__.items():
            if key not in ['name', 'msg', 'levelname', 'levelno', 'pathname', 'filename',
                           ...
                           'threadName', 'processName', 'process', 'args', 'asctime',
                           'correlation_id']: # Exclude standard and already included
...
class StructuredLogger:
    ...
    """
    def __init__(self, log_file_path: Path, logger_name: str = "ollash", log_level: str = "INFO", max_bytes: int = 10 * 1024 * 1024, backup_count: int = 5):
    ...
    def tool_call(self, tool_name: str, args: Dict[str, Any]):
        ...
        self._logger.info(f"Tool Call: {tool_name}", extra={"type": "tool_call", "tool_name": tool_name, "args": args})

    def tool_result(self, tool_name: str, result: Any, success: bool):
        """Logs a tool result event."""
        self._logger.info(f"Tool Result: {tool_name} {'SUCCESS' if success else 'FAILURE'}", extra={"type": "tool_result", "tool_name": tool_name, "result": result, "success": success})

    def llm_request(self, model: str, prompt_len: int, messages: List[Dict]):
        """Logs an LLM API request."""
        self._logger.info(f"LLM Request to {model}", extra={"type": "llm_request", "model": model, "prompt_length": prompt_len, "messages_hash": hash(json.dumps(messages))}) # Hash to avoid logging huge messages directly

    def llm_response(self, model: str, response_data: Dict, usage: Dict, latency: float, success: bool):
        """Logs an LLM API response."""
        self._logger.info(f"LLM Response from {model}", extra={"type": "llm_response", "model": model, "success": success, "latency_ms": latency * 1000, "usage": usage, "response_hash": hash(json.dumps(response_data))})
