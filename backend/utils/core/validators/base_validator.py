    ...
    def _run_linter_command(
        ...
        if not self.command_executor:
            return ValidationResult(
                ...
                "CommandExecutor not available for linting.",
                lines,
                chars,
            )
        ...
        if result.success:
            ...
            if (
                ...
                or "is not recognized" in error_message.lower()
                or "no such file or directory" in error_message.lower()
            ):
                return ValidationResult(
                    file_path,
                    ValidationStatus.SYNTAX_ERROR,
                    f"Linter command not found: {command[0]}. " + error_message,
                    lines,
                    chars,
                )
            ...
            if error_message:
                ...
                for line in error_message.splitlines():
                    if (
                        "no issues found" in line.lower()
                        or "completed" in line.lower()
                        or "checking" in line.lower()
                        or "passing" in line.lower()
                ...
                return ValidationResult(
                    file_path,
                    ValidationStatus.SYNTAX_ERROR,
                    f"Linter command failed with exit code {result.return_code}. No output.",
                    lines,
                    chars,
                )
