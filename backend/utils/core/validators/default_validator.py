...
class DefaultValidator(BaseValidator):
    ...
    def validate(
        ...
        return ValidationResult(
            ...
            "No specific validation, basic checks passed",
            lines,
            chars,
        )
        ...
        if self._is_html_partial(path):
            ...
            if not re.search(r"<\w+[^>]*>", lower):
                return ValidationResult(
                    ...
                    "HTML partial contains no recognizable HTML tags",
                    lines,
                    chars,
                )
            return ValidationResult(
    ...
    def _validate_shell(self, path, content, lines, chars) -> ValidationResult:
        ...
        if not first_line.startswith("#!") and not first_line.startswith("#"):
            return ValidationResult(
                path,
                ValidationStatus.VALID,
                "Shell script (no shebang, but content present)",
                lines,
                chars,
            )
        return ValidationResult(
    ...
    def _validate_dependency_file(
        ...
        return ValidationResult(
            path,
            ValidationStatus.VALID,
            "Dependency file basic check passed",
            lines,
            chars,
        )
    ...
    def _validate_requirements_txt(
        ...
        return ValidationResult(
            path,
            ValidationStatus.VALID,
            f"Requirements OK ({len(entries)} packages)",
            lines,
            chars,
        )
    ...
    def _validate_package_json_deps(
        ...
        return ValidationResult(
            path,
            ValidationStatus.VALID,
            f"package.json OK ({total_deps} deps)",
            lines,
            chars,
        )
