...
class LLMResponseParser:
    ...
    def remove_think_blocks(response):
        ...
        return re.sub(r"<(?:think|thinking_process)>[\s\S]*?(?:</(?:think|thinking_process)>|$)", "", response, flags=re.I).strip(), match.group(1).strip()

    @staticmethod
    def extract_raw_content(response):
        cleaned, _ = LLMResponseParser.remove_think_blocks(response)
        if not cleaned.strip(): return ""
        if "```" in cleaned: return LLMResponseParser.extract_single_code_block(cleaned)
    ...
    def extract_single_code_block(response):
        ...
        if cleaned.startswith("```"):
            ...
            if len(lines) > 0: cleaned = "\n".join(lines[1:])
        if cleaned.endswith("```"):
            lines = cleaned.splitlines()
            if len(lines) > 0 and lines[-1].strip() == "```": cleaned = "\n".join(lines[:-1])
        return cleaned.strip()
    ...
    def extract_multiple_files(response):
        cleaned, _ = LLMResponseParser.remove_think_blocks(response)
        files = {}; lines = cleaned.splitlines()
        content, name, in_block, pot_name = [], None, False, None
        for line in lines:
            stripped = line.strip()
            if not in_block:
                m = re.search(r"(?:#|//)[\s]*filename:\s*([^\n]+)", stripped)
                if m: pot_name = Path(m.group(1).strip()).as_posix().replace("..", "").lstrip("/"); continue
            if stripped.startswith("```"):
                if in_block:
                    ...
                    in_block = True
                    if pot_name: name, pot_name = pot_name, None
                    elif "# filename:" in line: name = Path(line.split("# filename:", 1)[1].strip()).as_posix().replace("..", "").lstrip("/")
            elif in_block:
        ...
        if in_block and name: files[name] = "\n".join(content).strip()
        return files
