...
from backend.utils.core.validators.base_validator import (ValidationResult,
                                                          ValidationStatus)
from backend.utils.core.validators.default_validator import DefaultValidator
from backend.utils.core.validators.javascript_validator import \
    JavascriptValidator
...
from backend.utils.core.validators.typescript_validator import \
    TypescriptValidator
...
class FileValidator:
    ...
    def __init__(self, logger=None, command_executor=None):
        ...
        self._javascript_validator = JavascriptValidator(
            logger=logger, command_executor=command_executor
        )
    ...
    def validate(self, file_path: str, content: str) -> ValidationResult:
        ...
        if char_count == 0:
            return ValidationResult(
                file_path, ValidationStatus.EMPTY, "File is empty", 0, 0
            )

        # Perform basic validation first
        basic_result = self._default_validator.basic_validation(
            file_path, stripped, ext, line_count, char_count
        )
        ...
        if validator:
            ...
            if Path(file_path).name in self._default_validator.DEPENDENCY_FILES:
                return self._default_validator.validate(
                    file_path, stripped, line_count, char_count, ext
                )
