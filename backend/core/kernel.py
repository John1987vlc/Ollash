...
class AgentKernel:
    ...
    def __init__(
        ...
        if structured_logger:
            ...
            self._structured_logger = StructuredLogger(
                log_file_path=log_dir / "kernel.log"
            )
        ...
        self._agent_logger = AgentLogger(
            structured_logger=self._structured_logger, logger_name="AgentKernel"
        )
        config_loader_logger = AgentLogger(
            structured_logger=self._structured_logger, logger_name="ConfigLoader"
        )
    ...
    def get_llm_models_config(self) -> LLMModelsConfig:
        ...
        if not isinstance(config, LLMModelsConfig):
            raise RuntimeError(
                "LLM models configuration not loaded or is of incorrect type."
            )
        return config
    ...
    def get_agent_features_config(self) -> AgentFeaturesConfig:
        ...
        if not isinstance(config, AgentFeaturesConfig):
            raise RuntimeError(
                "Agent features configuration not loaded or is of incorrect type."
            )
        return config
    ...
    def get_tool_settings_config(self) -> ToolSettingsConfig:
        ...
        if not isinstance(config, ToolSettingsConfig):
            raise RuntimeError(
                "Tool settings configuration not loaded or is of incorrect type."
            )
        return config
